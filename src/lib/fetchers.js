/**
 * Deck fetchers for Moxfield, Archidekt, and TappedOut.
 * Each returns { deckName: string, cards: [{ name, quantity, ... }] }
 */

// these sites block direct browser requests, so we always go through a proxy
const ALWAYS_PROXY = ['moxfield.com', 'tappedout.net', 'deckbox.org']

const PROXIES = [
  url => `/proxy?url=${encodeURIComponent(url)}`,                          // Cloudflare Pages Function (production)
  url => `https://corsproxy.io/?url=${encodeURIComponent(url)}`,           // fallback for local dev
  url => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`,  // last resort
]
...
async function fetchMoxfield(url) {
  ...
  // all cards including commanders live in data.boards — Moxfield returns the full card
  // object so we skip the Scryfall enrichment step for these
...
async function fetchTappedOut(url) {
  ...
  // TappedOut embeds the decklist in the HTML rather than a dedicated API endpoint
  const res  = await safeFetch(`https://tappedout.net/mtg-decks/${slug}/?_=${Date.now()}`)
  const html = await res.text()

  // cards are rendered as <li class="member"> — no set or foil data in the HTML,
  // so Scryfall will enrich by name after import
